import { defineProperty } from '@agnes/cosmokit'
import type { Promisify } from '@agnes/cosmokit'
import { Context } from './context.js'
import { Fiber, FiberState } from './fiber.js'
import { registerSystemDispatcher } from './system-events.js'
...
export class EventsService {
  ...
  constructor(private ctx: Context) {
    ...
    registerSystemDispatcher(this, (source, type, args) => this.#dispatch(source, type, args, true))
   ...
   */
  #dispatch(source: Context, type: DispatchMode, args: any[], system: boolean) {
    const thisArg = typeof args[0] === 'object' || typeof args[0] === 'function' ? args.shift() : null
    const name: string = args.shift()
    const filter = source?.[Context.filter]
    const dispatchHooks = (this._hooks['internal/dispatch'] || [])
      .filter(hook => hook.global || !filter || filter.call(source, hook.ctx))
      .map(hook => hook.callback.bind(source))
    for (const callback of dispatchHooks) {
      callback(type, name, args, thisArg, system)
    }
    // `internal/dispatch` is the gate itself. A public attempt is observable
    // above, but never reaches the callbacks with attacker-controlled gate
    // arguments or recursively dispatches the gate.
    if (name === 'internal/dispatch') return []
    const listenerFilter = thisArg?.[Context.filter]
    return (this._hooks[name] || [])
      .filter(hook => hook.global || !listenerFilter || listenerFilter.call(thisArg, hook.ctx))
      .map(hook => hook.callback.bind(thisArg))
  }

  dispatch(type: DispatchMode, args: any[]) {
    const service: EventsService = (this as any)[symbols.original] ?? this
    return service.#dispatch(this.ctx, type, args, false)
  }
  ...
  on(name: string | symbol, listener: (...args: any) => any, options?: boolean | EventOptions) {
    ...
    const service: EventsService = (this as any)[symbols.original] ?? this
    for (const callback of service.#dispatch(
      this.ctx,
      'bail',
      [this.ctx, 'internal/listener', name, listener, options],
      true,
    )) {
      const result = callback(name, listener, options)
      if (isBailed(result)) return result
    }
...
export interface Events {
  ...
  /** Waterfall: a service provide is checked before any effect or store mutation. */
  'internal/provide'(this: Context, name: string, value: unknown, next: () => void): void
  ...
  /** Every event dispatch before delivery, including source and private framework provenance. */
  'internal/dispatch'(
    this: Context,
    mode: DispatchMode,
    name: string,
