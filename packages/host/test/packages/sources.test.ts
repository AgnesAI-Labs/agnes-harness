...
describe('parseSource', () => {
  ...
  it('accepts contained local forms and refuses traversal spellings', () => {
    ...
    expect(() => parseSource('market:kiwi/x@1.0.0')).toThrow(/E_DEP_MISSING/)
...
describe('hashDirectory', () => {
  ...
  it('refuses a linked root directory before hashing its contents', () => {
    const rootAlias = join(root, 'alias')
    const linkType = process.platform === 'win32' ? 'junction' : 'dir' // guards-allow-platform: Exercise Windows directory links without file-symlink privileges.
    symlinkSync(root, rootAlias, linkType)
    expect(() => hashDirectory(rootAlias)).toThrow(/symbolic link/)
  })

  it('refuses a file symlink instead of hashing outside the tree', () => {
...
describe('fetchSource', () => {
  ...
  it('fails closed on an escaping source symlink and leaves no destination or stage', async () => {
    ...
    const linkType = process.platform === 'win32' ? 'junction' : 'dir' // guards-allow-platform: Exercise Windows directory links without file-symlink privileges.
    symlinkSync(outside, join(cwd, 'pkg'), linkType)
    const into = join(dir, 'installed')
  ...
  it.each(['npm-symlink.tgz', 'npm-hardlink.tgz'])(
    ...
    async (archive) => {
      const bytes = readFileSync(
        new URL(`../../../package-manager/test/fixtures/${archive}`, import.meta.url),
      )
      const integrity = `sha512-${createHash('sha512').update(bytes).digest('base64')}`
      const exec: ExecFn = async (_command, args) => {
        if (args[0] !== 'pack') return { stdout: '{}' }
