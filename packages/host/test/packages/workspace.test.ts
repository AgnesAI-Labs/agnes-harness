...
import { emptyLock } from '../../src/packages/lockfile.js'
import {
  hashWorkspace,
  readDeployManifest,
  readProfileFragment,
  verifyWorkspace,
} from '../../src/packages/workspace.js'
...
describe('workspace trust primitives', () => {
  ...
  it('refuses the planned partner package id because the current profile schema cannot represent it', () => {
    ...
    expect(() => readProfileFragment(deployDir, readDeployManifest(deployDir))).toThrow(
  ...
  it('rejects the obsolete plan shape and prefix-smuggled traversal paths', () => {
    ...
    expect(() => readDeployManifest(deployDir)).toThrow(/E_EXT_LOAD/)
  ...
  it('rejects a profile fragment that escapes through a parent symlink', () => {
    ...
    // guards-allow-platform: the target is a real directory; Windows junctions preserve this escape.
