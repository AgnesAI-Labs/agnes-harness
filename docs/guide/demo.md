...
[项目首页](../../README.md) · [文档导航](../README.md) · [安装指南](install.md)
...
| 方式 | 适合谁 | 环境与结果 |
| --- | --- | --- |
| Web / CLI 体验 | 想查看界面和模型交互 | 完成[首次运行](quickstart.md)后，选择下方示例；模型调用可能计费 |
...
跟随[后端插件教程](../develop/backend.md)，安装、信任并启用 `hot-tool-plugin`。在已配置模型的会话中要求调用 `demo_text_stats` 统计 `hello world`。
...
跟随[前端面板教程](../develop/frontend.md)加载 `client-panel/v1`。在 Web 中应看到 `Agnes client module demo · v1`；更新到 v2 后检查版本文字，停用后检查内置侧栏是否恢复。
...
跟随[前后端联动教程](../develop/fullstack.md)，在独立实验 profile 中配置服务能力、安装联动包，并先选择一个会话。
...
先完成[构建前提](install.md)。在源码仓库根运行；已有完整构建时可直接运行第二条。已有独立构建通过包括插件删除在内的自动演示，具体版本与结果见[验证记录](../maintainers/verification.md)。后续源码变化需要重新构建和验证。
...
任一步失败时，保留日志与结果目录，按[排错](troubleshooting.md)定位。当前脚本使用 `/tmp` 与 Unix socket；已有进程验证在 macOS 执行，不作为 Windows 验收命令。不要手工移除快照或强行释放仍被引用的 pin。
...
完整阶段与基线见[验证记录](../maintainers/verification.md)，可读代码见[演示脚本](../../tools/public-docs/smoke.mjs)。
...
下一步：[开发你自己的插件](../develop/plugins.md) · [把示例带到 FDE 场景](why-agh.md) · [反馈问题与开发协作](../develop/contributing.md)。
