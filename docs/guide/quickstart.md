...
[文档导航](../README.md) · 前提：[已完成构建](install.md)

完成这一页后，你将配置一个模型、运行一个小任务，并从 CLI 找到 Web 中创建的会话。前提是已完成[源码构建](install.md)，建议首次试用采用其中的独立 home。没有模型账号时，可以先体验[本地模拟模型演示](demo.md#不配置模型账号先跑通本地链路)。
...
没有可用模型时会进入 Provider 设置。选择账号/Provider，核对 Base URL，在密码输入框输入 API key，执行“测试连接”，选择返回目录中的模型，再保存。页面只显示已配置状态，不会读回已保存的明文密钥。测试连接和后续对话可能访问提供方并产生费用；无模型账号时请用[本地演示](demo.md)。
...
这是一条用户意图，执行边界由审批与策略决定。读取[安全说明](security.md)后再授予执行权限。

**如何确认跑通：** 页面出现本轮回答；若模型使用了工具，展开记录检查名称、参数和结果；确认任务进入完成或明确的等待状态。随后回到会话列表，再打开这条任务，核对历史仍可读取。遇到错误时保留错误码，按[排错](troubleshooting.md)继续。
...
想让 Agent 采用团队的方法，添加一个[Skill](skills.md)；想接入业务系统，从[插件开发](../develop/plugins.md)开始；想继续已有任务，阅读[会话与恢复](sessions.md)。
