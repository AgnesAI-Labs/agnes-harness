...
[文档导航](../README.zh-CN.md) · 前提：[Cordis 边界](plugins.zh-CN.md)
...
先保留包结构与注册方式，只替换业务逻辑；再根据实际副作用更新参数校验、权限、审批和重放声明。把连接与监听器加入 Cordis 生命周期，确保禁用后可以清理。需要展示业务结果时，继续[前端面板](frontend.zh-CN.md)和[联动教程](fullstack.zh-CN.md)。
