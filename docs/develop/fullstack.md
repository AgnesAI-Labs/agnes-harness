...
[文档导航](../README.md) · 前提：[后端](backend.md)与[前端](frontend.md)
...
后续 trust/enable 使用实际预览摘要，流程见[插件管理](../guide/packages.md)。
...
该组合覆盖示例模块、Host 装配撤权和 daemon 浏览器调用边界；完整 browser → BFF → daemon → worker → Host 的可选验收程序为 [web-workbench.mjs](../../tools/acceptance/web-workbench.mjs)，实际运行版本与覆盖范围单列于[验证记录](../maintainers/verification.md)。
