...
[文档导航](../README.zh-CN.md)

## 本仓许可边界

AGH 项目自有代码采用 **Apache License 2.0**，完整条款见根目录 [LICENSE](../../LICENSE)。[NOTICE](../../NOTICE) 汇总已识别的第三方归属与独立许可声明。

第三方组件、改编文件及部分示例保留各自的许可。根许可证不覆盖或改写这些独立声明；包含 Apache-2.0 与 MIT 内容的 Base、Host 和 package-manager 包在 manifest 中标记为 `Apache-2.0 AND MIT`。
...
使用或再分发相关内容时，请遵守其适用条款，保留随附的许可证、版权及归属声明。单独分发示例包或构建产物时，还需核对其中实际包含的依赖、素材及许可文本；发行前的核对步骤见[发布检查](release.zh-CN.md)。
